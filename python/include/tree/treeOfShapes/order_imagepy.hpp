...
void bindOrderImageResult(py::module &m, const std::string &type)
{
  ...
  py::class_<mt::OrderImageResult<T>>(m, className.c_str())
    .def(py::init<>())
    .def_readonly("orderImg", &mt::OrderImageResult<T>::orderImg)
    .def_readonly("R", &mt::OrderImageResult<T>::R)
    .def_readonly("flattern", &mt::OrderImageResult<T>::flattern)
    .def_readonly("domain", &mt::OrderImageResult<T>::domain);
